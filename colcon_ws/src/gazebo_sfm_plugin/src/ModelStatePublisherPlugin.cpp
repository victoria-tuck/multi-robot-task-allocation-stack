...
#include <gazebo_sfm_plugin/ModelStatePublisherPlugin.hpp>
...
class ModelStatePublisherPluginPrivate
{
public:
...
ModelStatePublisherPlugin::ModelStatePublisherPlugin()
: impl_(std::make_unique<ModelStatePublisherPluginPrivate>())
{
}
...
// Load the controller
void ModelStatePublisherPlugin::Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
{
  // Configure the plugin from the SDF file
//   impl_->ros_node_ = gazebo_ros::Node::Get(sdf, model);
  impl_->ros_node_ = gazebo_ros::Node::Get(sdf);
  impl_->model = model;
  impl_->world = model->GetWorld();
  ignition::math::Vector3d modelPos = model->WorldPose().Pos();
  std::cout << "********************  Initialzing Model " << model->GetName() << " at " << modelPos << std::endl;

 
  // Listen to the update event. This event is broadcast every simulation iteration
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&ModelStatePublisherPluginPrivate::OnUpdate, impl_.get(), std::placeholders::_1));
}

// Update the controller
void ModelStatePublisherPluginPrivate::OnUpdate(const gazebo::common::UpdateInfo & info)
{
...
ignition::math::Vector3d modelPos = model->WorldPose().Pos();
  ...
  pose_msg.child_frame_id = model->GetName();//link_->GetName();

  // Get inertial rates
  ignition::math::Vector3d vpos = model->WorldLinearVel(); //link_->WorldLinearVel();
  ignition::math::Vector3d veul = model->WorldAngularVel(); //link_->WorldAngularVel();

  // Get pose/orientation
  auto pose = model->WorldPose();//.Pos();//link_->WorldPose();

  // Fill out messages
  pose_msg.pose.pose.position = gazebo_ros::Convert<geometry_msgs::msg::Point>(pose.Pos());
  ignition::math::Vector3d modelPos2 = pose.Pos();
...
GZ_REGISTER_MODEL_PLUGIN(ModelStatePublisherPlugin) //GazeboRosP3DTest
