...
def talker():
    ...
    controls = np.zeros((2,num_human))
    controls[0,0] = 0.0; controls[1,0] = 1.0;
    controls[0,1] = 0.0; controls[1,1] = 0.5;
    controls[0,2] = 0.0; controls[1,2] = 0.5;
    controls[0,3] = 0.0; controls[1,3] = 1.0;
    controls[0,4] = 0.0; controls[1,4] = 1.0;
    controls[0,5] = 0.5; controls[1,5] = 0.5;
    controls[0,6] = 0.5; controls[1,6] = 0.5;
    controls[0,7] = 0.5; controls[1,7] = 0.5;
    controls[0,8] = 0.5; controls[1,8] = 0.5;
    controls[0,9] = 0.5; controls[1,9] = 0.5;
    ...
    while not rospy.is_shutdown():
        for i in range(num_human):
            ...
            des_heading = np.arctan2( controls[1,i], controls[0,i] )
    ...
    while not rospy.is_shutdown():
        ...
        if (alternate == False):
            ...
            for i in range(num_human):
                ...
                vel = Twist()
                vel.linear.x = np.linalg.norm( controls[:,i] )
                vel.linear.y = 0.0
                vel.linear.z = 0.0            
                des_heading = np.arctan2( controls[1,i], controls[0,i] )
                vel.angular.x = 0.0
